...
class TinyAyaAudioCausalLMOutputWithPast(ModelOutput):
    r"""
...
class TinyAyaAudioAttention(nn.Module):
    ...
    def __init__(
        ...
        config: TinyAyaAudioConfig | None = None,
    ):
        super().__init__()
...
class TinyAyaAudioEncoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: TinyAyaAudioConfig):
        super().__init__()
        self.embed_dim = config.d_model

        self.self_attn = TinyAyaAudioAttention(
...
@auto_docstring
class TinyAyaAudioPreTrainedModel(PreTrainedModel):
    config: TinyAyaAudioConfig
    base_model_prefix = "model"
    input_modalities = ("audio", "text")
    supports_gradient_checkpointing = True
    _no_split_modules = ["TinyAyaAudioAttention"]
...
class TinyAyaAudioEncoder(TinyAyaAudioPreTrainedModel):
    """
    ...
    # Ignore copy
    config: TinyAyaAudioEncoderConfig
    main_input_name = "input_features"
    input_modalities = "audio"
    _no_split_modules = ["TinyAyaAudioEncoderLayer"]

    def __init__(self, config: TinyAyaAudioEncoderConfig):
        ...
        self.layers = nn.ModuleList([TinyAyaAudioEncoderLayer(config) for _ in range(config.encoder_layers)])
    ...
    def forward(
        ...
        **kwargs,
        ...
        if input_features.shape[-1] != expected_seq_length:
            raise ValueError(
                f"TinyAyaAudio expects the mel input features to be of length {expected_seq_length}, but found {input_features.shape[-1]}. Make sure to pad the input mel features to {expected_seq_length}."
            )
...
class TinyAyaAudioMultiModalProjector(nn.Module):
    def __init__(self, config: TinyAyaAudioConfig):
        super().__init__()
...
class TinyAyaAudioForConditionalGeneration(TinyAyaAudioPreTrainedModel, GenerationMixin):
    def __init__(self, config: TinyAyaAudioConfig):
        super().__init__(config)
        self.audio_tower = AutoModel.from_config(config.audio_config)  # Usually a `Qwen2AudioEncoder` instance

        self.multi_modal_projector = TinyAyaAudioMultiModalProjector(config)
    ...
    def forward(
        ...
        **kwargs,
    ) -> tuple | TinyAyaAudioCausalLMOutputWithPast:
        r"""
        ...
        return TinyAyaAudioCausalLMOutputWithPast(
            loss=loss,
            logits=logits,
...
__all__ = ["TinyAyaAudioForConditionalGeneration", "TinyAyaAudioPreTrainedModel", "TinyAyaAudioEncoder"]
