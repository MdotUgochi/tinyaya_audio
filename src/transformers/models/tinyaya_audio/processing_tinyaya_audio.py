...
"""
Processor class for TinyAyaAudio.
"""
...
class TinyAyaAudioProcessorKwargs(ProcessingKwargs, total=False):
    _defaults = {
        "text_kwargs": {
            "padding": False,
        },
...
@auto_docstring
class TinyAyaAudioProcessor(ProcessorMixin):
    ...
    def __init__(
        ...
        if self.audio_token_id == tokenizer.unk_token_id:
            import warnings
            warnings.warn(
                f"<|AUDIO|> token not found in Cohere tokenizer vocabulary — got unk_token_id={self.audio_token_id}. "
                "Make sure to call tokenizer.add_special_tokens({'additional_special_tokens': ['<|AUDIO|>', '<|audio_bos|>', '<|audio_eos|>']}) "
                "and model.language_model.resize_token_embeddings(len(tokenizer)) before using this processor.",
                UserWarning,
            )
    ...
    def __call__(
        ...
        **kwargs: Unpack[TinyAyaAudioProcessorKwargs],
    ) -> BatchFeature:
        if text is None:
